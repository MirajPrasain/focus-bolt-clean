    ...
    try:
        ...
        while True:
            try:
                ...
                success, frame = decode_image_safely(image_data)
                
                if not success:
                    logger.warning(f"Frame {frame_count}: Skipping malformed frame")
                    continue
                
                # Process frame and send score
                try:
                    score = process_frame(frame, current_timestamp)
                    await websocket.send_text(str(score))
                    logger.debug(f"Frame {frame_count}: Score {score}, Timestamp: {current_timestamp:.2f}s")
                    
                except Exception as e:
                    logger.error(f"Error processing frame {frame_count}: {e}")
                    # Send a neutral score instead of crashing
